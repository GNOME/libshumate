/* shumate-kinetic-scroll-view.c: Finger scrolling container actor
 *
...
G_DEFINE_TYPE (ShumateKineticScrollView, shumate_kinetic_scroll_view, CLUTTER_TYPE_ACTOR)

#define GET_PRIVATE(o) \
  (G_TYPE_INSTANCE_GET_PRIVATE ((o), SHUMATE_TYPE_KINETIC_SCROLL_VIEW, ShumateKineticScrollViewPrivate))
...
button_release_event_cb (ClutterActor *stage,
    ...
    ShumateKineticScrollView *scroll);

static void
shumate_kinetic_scroll_view_get_property (GObject *object, guint property_id,
    GValue *value, GParamSpec *pspec)
{
  ShumateKineticScrollViewPrivate *priv = SHUMATE_KINETIC_SCROLL_VIEW (object)->priv;
...
static void
shumate_kinetic_scroll_view_set_property (GObject *object, guint property_id,
...
static void
shumate_kinetic_scroll_view_dispose (GObject *object)
{
  ShumateKineticScrollViewPrivate *priv = SHUMATE_KINETIC_SCROLL_VIEW (object)->priv;
  ...
  G_OBJECT_CLASS (shumate_kinetic_scroll_view_parent_class)->dispose (object);
}


static void
shumate_kinetic_scroll_view_finalize (GObject *object)
{
  ShumateKineticScrollViewPrivate *priv = SHUMATE_KINETIC_SCROLL_VIEW (object)->priv;

  g_array_free (priv->motion_buffer, TRUE);

  G_OBJECT_CLASS (shumate_kinetic_scroll_view_parent_class)->finalize (object);
}


static void
shumate_kinetic_scroll_view_class_init (ShumateKineticScrollViewClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  g_type_class_add_private (klass, sizeof (ShumateKineticScrollViewPrivate));

  object_class->get_property = shumate_kinetic_scroll_view_get_property;
  object_class->set_property = shumate_kinetic_scroll_view_set_property;
  object_class->dispose = shumate_kinetic_scroll_view_dispose;
  object_class->finalize = shumate_kinetic_scroll_view_finalize;

  g_object_class_install_property (object_class,
      PROP_MODE,
      g_param_spec_boolean ("mode",
          "ShumateKineticScrollViewMode",
          "Scrolling mode",
          FALSE,
          G_PARAM_READWRITE));
...
static void
clamp_adjustments (ShumateKineticScrollView *scroll)
{
  ShumateKineticScrollViewPrivate *priv = scroll->priv;

  if (priv->viewport)
    {
      ShumateAdjustment *hadj, *vadj;
      gdouble d, value, lower, step_increment;

      shumate_viewport_get_adjustments (SHUMATE_VIEWPORT (priv->viewport),
          &hadj, &vadj);

      /* Snap to the nearest step increment on hadjustment */

      shumate_adjustment_get_values (hadj, &value, &lower, NULL,
          &step_increment);
      d = (rint ((value - lower) / step_increment) *
           step_increment) + lower;
      shumate_adjustment_set_value (hadj, d);

      /* Snap to the nearest step increment on vadjustment */
      shumate_adjustment_get_values (vadj, &value, &lower, NULL,
          &step_increment);
      d = (rint ((value - lower) / step_increment) *
           step_increment) + lower;
      shumate_adjustment_set_value (vadj, d);
    }
}
      ...
      if (priv->viewport)
        {
          ...
          motion = &g_array_index (priv->motion_buffer,
                ShumateKineticScrollViewMotion, priv->last_motion);
          if (hadjust)
            {
              dx = (motion->x - x) +
                shumate_adjustment_get_value (hadjust);
              shumate_adjustment_set_value (hadjust, dx);
            }

          if (vadjust)
            {
              dy = (motion->y - y) +
                shumate_adjustment_get_value (vadjust);
              shumate_adjustment_set_value (vadjust, dy);
            }
        }
  ...
  if (priv->viewport)
    {
      ...
      for (i = 0; i < clutter_timeline_get_delta (timeline) / 15; i++)
        {
          shumate_adjustment_set_value (hadjust,
              priv->dx +
              shumate_adjustment_get_value (hadjust));
          shumate_adjustment_set_value (vadjust,
              priv->dy +
              shumate_adjustment_get_value (vadjust));
      ...
      shumate_adjustment_get_values (hadjust, &value, &lower, &upper,
          NULL);
      ...
      if (stop)
        {
          shumate_adjustment_get_values (vadjust, &value, &lower, &upper,
              NULL);
      ...
      if (clutter_actor_transform_stage_point (actor,
              ...
              &x, &y))
        {
          ...
          for (i = 0; i < priv->last_motion; i++)
            {
              ShumateKineticScrollViewMotion *motion =
                &g_array_index (priv->motion_buffer, ShumateKineticScrollViewMotion, i);
          ...
          if (time_diff != 0)
            {
              ...
              if (ABS (priv->dx) > 1 ||
                  ABS (priv->dy) > 1)
                {
                  ...
                  /* Solving for dx */
                  d = a * priv->dx;
                  shumate_adjustment_get_values (hadjust, &value, &lower, NULL,
                      &step_increment);
                  ...
                  /* Solving for dy */
                  d = a * (priv->dy);
                  shumate_adjustment_get_values (vadjust, &value, &lower, NULL,
                      &step_increment);
                  d = ((rint (((value + d) - lower) / step_increment) *
                        step_increment) + lower) - value;
                  priv->dy = (d / a);
    ...
    ShumateKineticScrollView *scroll)
{
  ...
  if (event->type == CLUTTER_TOUCH_BEGIN && priv->sequence)
    {
      ...
      shumate_kinetic_scroll_view_stop (scroll);
      clamp_adjustments (scroll);
      g_signal_emit_by_name (scroll, "panning-completed", NULL);
  ...
  if ((((event->type == CLUTTER_BUTTON_PRESS) && (bevent->button == 1)) ||
      ...
      stage)
    {
      ...
      /* Reset motion buffer */
      priv->last_motion = 0;
      motion = &g_array_index (priv->motion_buffer, ShumateKineticScrollViewMotion, 0);
...
static void
shumate_kinetic_scroll_view_init (ShumateKineticScrollView *self)
{
  ShumateKineticScrollViewPrivate *priv = self->priv = GET_PRIVATE (self);

  priv->motion_buffer = g_array_sized_new (FALSE, TRUE,
        sizeof (ShumateKineticScrollViewMotion), 3);
...
ClutterActor *
shumate_kinetic_scroll_view_new (gboolean kinetic,
    ShumateViewport *viewport)
{
  ClutterActor *scroll_view;

  scroll_view = CLUTTER_ACTOR (g_object_new (SHUMATE_TYPE_KINETIC_SCROLL_VIEW,
          "mode", kinetic, NULL));
  SHUMATE_KINETIC_SCROLL_VIEW (scroll_view)->priv->viewport = CLUTTER_ACTOR (viewport);
...
void
shumate_kinetic_scroll_view_stop (ShumateKineticScrollView *scroll)
{
  ShumateKineticScrollViewPrivate *priv;

  g_return_if_fail (SHUMATE_IS_KINETIC_SCROLL_VIEW (scroll));
